...
module Monaco =
    ...
    type [<AllowNullLiteral>] TValueCallback<'T> =
        [<Emit "$0($1...)">] abstract Invoke: value: U2<'T, PromiseLike<'T>> -> unit
    ...
    type [<AllowNullLiteral>] PromiseStatic =
        [<Emit "new $0($1...)">] abstract Create: executor: ((U2<'T, PromiseLike<'T>> -> unit) -> (obj option -> unit) -> ('TProgress -> unit) -> unit) * ?oncancel: (unit -> unit) -> Promise<'T, 'TProgress>
        // abstract ``as``: value: obj -> Promise<obj>
        // abstract ``as``: value: obj -> Promise<obj>
        // abstract ``as``: value: PromiseLike<'T> -> PromiseLike<'T>
