...
- Fable.PowerPack
    ...
    - Fetch API
NOTE:
The API in Fable's REPL may differ from nuget libraries for:
    - Fable.Elmish
    - Fable.React
    - Fable.PowerPack
...
let private getRandomUser () =
    ...
    |> Promise.bind(fun _ ->
        ...
        |> Promise.map(fun text ->
            ...
            Decode.fromString resultDecoder text
        )
    )
